...
import com.example.luck_rent_android.databinding.ActivityTenantContactBinding
...
import com.example.luck_rent_android.ui.main.order.OrderItem
...
class TenantContact : BaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    private var binding: ActivityTenantContactBinding? = null
    private lateinit var getValue: String
    private var mDataSideItem = ArrayList<SideItems>()
    private var mTopDataSideItem = ArrayList<SideItems>()

    override fun onCreate(savedInstanceState: Bundle?) {
        binding = DataBindingUtil.setContentView(this, R.layout.activity_tenant_contact)
        ...
        getValue = intent.getStringExtra("Tenant").toString()
        when (getValue) {
            "TenantContact" -> {
                binding?.topBar?.tvText?.text = "Tenant"
                binding?.rlAutomatic?.visible()
            }

            "EmergencyContact" -> {
                binding?.topBar?.tvText?.text = "Tenant Emergency"
            }
            "ManagerContact" -> {
                binding?.topBar?.tvText?.text = "Starata Manager"
            }
            "CareTakerContact" -> {
                binding?.topBar?.tvText?.text = "Care Taker"
            }
            "ContractorContact" -> {
                binding?.topBar?.tvText?.text = "Contractor"
            }
            "OtherContact" -> {
                binding?.topBar?.tvText?.text = "Other"
            }
        }
